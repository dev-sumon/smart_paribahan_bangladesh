    ...
    <div class="container">
        ...
        <div class="d-flex d-lg-none align-items-center justify-content-center responsive_menu">
            ...
            @elseif (Auth::guard('field_worker')->check())
                <a
                    href="{{ route('field_worker.dashboard', Auth::guard('field_worker')->user()->id) }}">{{ __('Profile') }}</a>
            @else
                {{-- Desktop View --}}
                <a href="{{ route('f.login.index') }}"
                    class="btn btn-outline-danger me-2 d-none d-lg-block login loginButton">{{ __('Log In') }}</a>
                <a href="{{ route('f.signup.index') }}"
                    class="btn btn-outline-danger d-none d-lg-block signUp">{{ __('Sign Up') }}</a>

                {{-- Mobile View --}}
                <div class="d-flex d-lg-none align-items-center justify-content-center responsive_menu">
                    <a href="{{ route('f.login.index') }}" class="btn btn-outline-danger btn-sm btn-login">
                        <i class="fas fa-user-circle"></i> {{ __('Log In') }}
                    </a>
            ...
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarContent"
                ...
                <span><i class="fa-solid fa-bars"></i></a></span>
            </button>
        </div>
        ...
        <div class="collapse navbar-collapse justify-content-between nav_bar" id="navbarContent">
            ...
            <div class="d-flex align-items-center">
                <div class="dropdown me-3">
                    <a class="btn btn-outline-secondary dropdown-toggle language_button" href="#"
                    ...
                    <ul class="dropdown-menu" aria-labelledby="languageDropdown">
                        ...
                        <li><a class="dropdown-item"
                                href="{{ request()->fullUrlWithQuery(['lang' => 'bn']) }}">বাংলা</a></li>
                    </ul>
                </div>
                @if (Auth::guard('driver')->check())
                ...
                @elseif (Auth::guard('field_worker')->check())
                    <a href="{{ route('field_worker.dashboard', Auth::guard('field_worker')->user()->id) }}">
                        <img src="{{ asset('storage/' . Auth::guard('field_worker')->user()->image ?? 'default/profile.png') }}"
                            alt="Profile" class="rounded-circle" width="40" height="40">
                    </a>
                @else
                    <a href="{{ route('f.login.index') }}" class="btn btn-outline-danger me-2 d-none d-lg-block login loginButton">
                        {{ __('Log In') }}
                    </a>
                    <a href="{{ route('f.signup.index') }}" class="btn btn-outline-danger d-none d-lg-block signUp">
                        {{ __('Sign Up') }}
                    </a>
                @endif
            </div>
