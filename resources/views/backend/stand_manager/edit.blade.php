...
@section('title', 'Admin - Stand Manager Update')
@section('content')
                ...
                <div class="card">
                    <div class="card-header">
                        <span class="float-left card-title">
                            <h4>{{ __('Stand Manager Update') }}</h4>
                        </span>
                        <span class="float-right">
                            <a href="{{ route('manager.index') }}" class="btn btn-info">{{ __('Back') }}</a>
                        </span>
                    </div>
                    <div class="card-body">
                        <div class="row">
                                ...
                                <form action="{{ route('manager.update', $manager->id) }}" method="POST"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="title"
                                            placeholder="Enter The Manager Name" name="title"
                                            value="{{ old('title') ?? $manager->title }}" oninput="slugGenerate($(this))">
                                        @if ($errors->has('title'))
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="email" name="email" value="{{ old('email') ?? $manager->email }}"
                                            class="form-control" placeholder="Enter manager Email">
                                        @if ($errors->has('email'))
                                    ...
                                    <div class="form-group">
                                        <label for="phone" class="mt-3">{{ __('Phone No.') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="tel" name="phone" value="{{ old('phone') ?? $manager->phone }}"
                                            class="form-control" placeholder="Enter manager Phone">
                                        @if ($errors->has('phone'))
                                        ...
                                        <select name="blood_group_id" id="blood_group_id" class="form-control">
                                            ...
                                            @foreach ($bloods as $blood)
                                                <option value="{{ $blood->id }}"
                                                    {{ old('blood_group_id', $manager->blood_group_id) == $blood->id ? 'selected' : '' }}>
                                                    {{ $blood->blood_group }}
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="division_id" id="division" class="form-control">
                                            ...
                                            @foreach ($divisions as $division)
                                                <option value="{{ $division->id }}"
                                                    {{ $manager->division_id == $division->id ? 'selected' : '' }}>
                                                    {{ $division->division }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="district_id" id="district" class="form-control">
                                            ...
                                            @foreach ($districts as $district)
                                                <option value="{{ $district->id }}"
                                                    {{ $manager->district_id == $district->id ? 'selected' : '' }}>
                                                    {{ $district->district }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="thana_id" id="thana" class="form-control">
                                            ...
                                            @foreach ($thanas as $thana)
                                                <option value="{{ $thana->id }}"
                                                    {{ $manager->thana_id == $thana->id ? 'selected' : '' }}>
                                                    {{ $thana->thana }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="union_id" id="union" class="form-control">
                                            ...
                                            @foreach ($unions as $union)
                                                <option value="{{ $union->id }}"
                                                    {{ $manager->union_id == $union->id ? 'selected' : '' }}>
                                                    {{ $union->union }}
                                                </option>
                                            @endforeach
                                        </select>
                                            ...
                                            <select name="stand_id" id="stand" class="form-control">
                                                ...
                                                @foreach ($stands as $stand)
                                                    <option value="{{ $stand->id }}"
                                                        {{ $manager->stand_id == $stand->id ? 'selected' : '' }}>
                                                        {{ $stand->title }}
                                                    </option>
                                                @endforeach
                                            </select>
