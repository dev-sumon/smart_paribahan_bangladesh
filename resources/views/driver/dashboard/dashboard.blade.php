                        ...
                        <form action="{{ route('signup.update', $driver->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" style="border: 2px solid #ea1827"
                                    placeholder="আপনার নাম লিখুন" name="name" value="{{ old('name') ?? $driver->name }}">
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="description"
                                    style="border: 2px solid #ea1827" placeholder="আপনার সম্পর্কে লিখুন" name="description"
                                    value="{{ old('description') ?? $driver->description }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="designation"
                                    style="border: 2px solid #ea1827" placeholder="আপনার পদবি লিখুন" name="designation"
                                    value="{{ old('designation') ?? $driver->designation }}">
                                @if ($errors->has('designation'))
                                    <div class="text-danger">{{ $errors->first('designation') }}</div>
                                @endif
                            </div>
                            <div class="form-group">
                                <label for="driving_license">{{ __('ড্রাইভিং লাইসেন্স') }} <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="driving_license"
                                    style="border: 2px solid #ea1827" placeholder="আপনার ড্রাইভিং লাইসেন্স নাম্বার লিখুন"
                            ...
                            <div class="form-group">
                                <label for="email" class="mt-3">{{ __('মেইল লিখুন') }} <span
                                        class="text-danger">*</span></label>
                                <input type="email" name="email" style="border: 2px solid #ea1827"
                                    value="{{ old('email') ?? $driver->email }}" class="form-control"
                            ...
                            <div class="form-group">
                                <label for="phone" class="mt-3">{{ __('মোবাইল নাম্বার') }} <span
                                        class="text-danger">*</span></label>
                                <input type="tel" name="phone" style="border: 2px solid #ea1827" class="form-control"
                                    placeholder="মোবাইল নাম্বার" value="{{ $driver->phone ?? old('phone') }}">
                                @if ($errors->has('phone'))
                            ...
                            <div class="form-group">
                                ...
                                @if ($driver->image)
                                    <img src="{{ asset('storage/' . $driver->image) }}" alt="Driver Image"
                            ...
                            <div class="form-group">
                                ...
                                <select name="vehicle_id" id="Stand" class="form-select select_iteam"
                                    style="border: 2px solid #ea1827">
                            ...
                            <div class="form-group">
                                ...
                                <select name="district_id" id="district" class="form-control" style="border: 2px solid #ea1827">
                                    ...
                                    @foreach ($districts as $district)
                                        <option value="{{ $district->id }}" {{ $driver->district_id == $district->id ? 'selected' : '' }}>
                                            {{ $district->district }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="thana_id" id="thana" class="form-control" style="border: 2px solid #ea1827">
                                    ...
                                    @foreach ($thanas as $thana)
                                        <option value="{{ $thana->id }}" {{ $driver->thana_id == $thana->id ? 'selected' : '' }}>
                                            {{ $thana->thana }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="union_id" id="union" class="form-control" style="border: 2px solid #ea1827">
                                    ...
                                    @foreach ($unions as $union)
                                        <option value="{{ $union->id }}" {{ $driver->union_id == $union->id ? 'selected' : '' }}>
                                            {{ $union->union }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="row">
                    <div class="col-md-2 m-auto">
                        <form id="logout-form" action="{{ route('driver.logout') }}" method="POST"
                            ...
                            @csrf
                            <div class="form-group">
                                <a class="dropdown-item text-center" href="{{ route('driver.logout') }}"
...
@push('script')
    <script>
        $(document).ready(function() {
            // Division পরিবর্তন হলে জেলা লোড হবে
            $('#division').on('change', function() {
                var division_id = $(this).val();
                if (division_id) {
                    let _url = '{{ route('driver.getDistricts', ['division_id' => ':id']) }}'.replace(
                        ':id', division_id);
                    $.ajax({
                        url: _url,
                        type: 'GET',
            ...
            // District পরিবর্তন হলে থানা লোড হবে
            $('#district').on('change', function() {
                var district_id = $(this).val();
                if (district_id) {
                    let _url = '{{ route('driver.getThanas', ['district_id' => ':id']) }}'.replace(':id',
                        district_id);
                    $.ajax({
                        url: _url,
                        type: 'GET',
            ...
            // Thana পরিবর্তন হলে Union লোড হবে
            $('#thana').on('change', function() {
                var thana_id = $(this).val();
                if (thana_id) {
                    let _url = '{{ route('driver.getUnions', ['thana_id' => ':id']) }}'.replace(':id',
                        thana_id);
                    $.ajax({
                        url: _url,
                        type: 'GET',
            ...
            // Union পরিবর্তন হলে Stand লোড হবে
            $('#union').on('change', function() {
                var union_id = $(this).val();
                if (union_id) {
                    let _url = '{{ route('driver.getStands', ['union_id' => ':id']) }}'.replace(':id',
                        union_id);
                    $.ajax({
                        url: _url,
                        type: 'GET',
