                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <span class="float-right">
                            <a href="{{ route('field_worker.union.index') }}" class="btn btn-info">{{ __('Back') }}</a>
                        </span>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-10 m-auto">
                                <form action="{{ route('field_worker.union.update', $union->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <select name="district_id" id="district" class="form-control">
                                            ...
                                            @foreach ($districts as $district)
                                                <option value="{{ $district->id }}"
                                                    {{ $union->district_id == $district->id ? 'selected' : '' }}>
                                                    {{ $district->district }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="thana_id" id="thana" class="form-control">
                                            ...
                                            @foreach ($thanas as $thana)
                                                <option value="{{ $thana->id }}"
                                                    {{ $union->thana_id == $thana->id ? 'selected' : '' }}>
                                                    {{ $thana->thana }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group mt-3">
                                        ...
                                        <input type="text" class="form-control" id="union"
                                            placeholder="Enter The Union Name" name="union"
                                            value="{{ old('union') ?? $union->union }}">
                                        @if ($errors->has('union'))
                                    ...
                                    <div class="form-group mt-3">
                                        ...
                                        <select name="status" id="status" class="form-control">
                                            <option value="1"
                                                {{ (old('status') ?? $union->status) == 1 ? 'selected' : '' }}>
                                                {{ __('Active') }}</option>
                                            <option value="0"
                                                {{ (old('status') ?? $union->status) == 0 ? 'selected' : '' }}>
                                                {{ __('Deactive') }}</option>
                                        </select>
